...
export async function POST(req: Request) {
  ...
  const requestedToTierRaw =
    typeof (body as any)?.toTier === 'string' ? String((body as any).toTier) : ''
  const requestedToTier = requestedToTierRaw.trim().toLowerCase()
  if (!['starter', 'professional', 'business'].includes(requestedToTier)) {
    return badRequest('toTier must be starter, professional, or business')
  }
  ...
  const currentTier =
    typeof account.subscription_tier === 'string'
      ? account.subscription_tier.trim().toLowerCase()
      : 'trial'
  ...
  try {
    const updated: any = await stripe.subscriptions.update(subscriptionId, {
      ...
      return ok({
        ...
        current_period_end: currentPeriodEnd,
        message: cancelAt
          ? `Plan change scheduled. Access remains active until ${new Date(cancelAt).toLocaleDateString(
              'en-US'
            )}.`
