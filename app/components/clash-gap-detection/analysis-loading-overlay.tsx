...
export function AnalysisLoadingOverlay({
  ...
  const hasDetail = Boolean(statusDetail?.trim())
  ...
  useEffect(() => {
    ...
    if (hasDetail || maxVisibleMs <= 0) {
      setAutoHidden(false)
      return
    }
    const id = window.setTimeout(() => setAutoHidden(true), maxVisibleMs)
    return () => window.clearTimeout(id)
  }, [open, maxVisibleMs, hasDetail])

  if (!open || forceHidden || autoHidden) return null
    ...
    >
      <div className="absolute inset-0 grid place-items-center px-6">
        <div className="relative flex w-full max-w-md flex-col items-center gap-6">
          ...
          {detail ? (
            <p className="max-w-sm text-center text-sm font-medium leading-snug text-[#334155]">
              {detail}
            </p>
          ) : null}
        </div>
