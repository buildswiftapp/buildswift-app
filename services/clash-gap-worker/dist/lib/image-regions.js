...
export function planSheetRegions(width, height) {
...
async function cropRegion(imageBuffer, region) {
    const img = await loadImage(imageBuffer);
    const w = Math.min(region.w, img.width - region.x);
    const h = Math.min(region.h, img.height - region.y);
    if (w <= 0 || h <= 0)
        throw new Error(`Invalid crop region ${region.label}`);
    const canvas = createCanvas(w, h);
    const ctx = canvas.getContext('2d');
    ctx.drawImage(img, region.x, region.y, w, h, 0, 0, w, h);
    return canvas.toBuffer('image/png');
}
function regionSection(label, text) {
    const header = `[${label.replace(/_/g, ' ').toUpperCase()}]`;
    return `${header}\n${text}`;
}
export async function ocrImageWithVisionRegions(imageBuffer, concurrency = 6, regions) {
    const img = await loadImage(imageBuffer);
    const layout = regions ?? planSheetRegions(img.width, img.height);
    const limit = pLimit(concurrency);
    const entries = await Promise.all(layout.map((region) => limit(async () => {
        try {
