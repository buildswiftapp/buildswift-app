...
import { markStageCompleted, setFileChunkStatus, setProgress, setStage, updateAnalysisStep, } from '../lib/stages.js';
import { clashGapImagePath, fetchAllRows } from '../lib/storage.js';
import { sb } from '../supabase.js';
function sha256Buffer(buffer) {
    return createHash('sha256').update(buffer).digest('hex');
}
async function loadCompletedPageIndexes(fileId) {
...
async function loadExistingRowIds(fileId) {
    const rows = await fetchAllRows(async (from, to) => sb()
        .from('clash_gap_extracted_sheets')
...
async function countRenderedPages(analysisId) {
...
async function tryCompleteChunkStage(analysisId) {
    ...
    for (const file of files) {
        ...
        if (isPdf && file.chunk_status === 'running')
            return;
        const expected = isImage ? 1 : file.page_count ?? 0;
...
export async function runChunkJob(input) {
    ...
    try {
        ...
        await setProgress(analysisId, 'chunk', totals.processed, Math.max(totals.total, totals.processed), `page ${totals.processed}/${Math.max(totals.total, totals.processed)}`);
