...
async function loadFiles(analysisId: string): Promise<FileRow[]> {
...
async function registerImageUploads(files: FileRow[]): Promise<number> {
  let registered = 0
...
async function pdfNeedsChunking(file: FileRow): Promise<boolean> {
  ...
  if (expected > 0 && (count ?? 0) >= expected) return false
  return true
}
...
export async function runChunkStageForAnalysis(params: {
  ...
  try {
    ...
    const pdfJobs: FileRow[] = []
    for (const file of processable) {
      if (!isPdfFile(file)) continue
      if (await pdfNeedsChunking(file)) pdfJobs.push(file)
    }
